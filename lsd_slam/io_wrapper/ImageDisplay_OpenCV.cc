	...
	while (imageThreadKeepRunning) {
		...
		while (displayQueue.size() > 0) {
			if (!displayQueue.back().autoSize) {
				if (openWindows.find(displayQueue.back().name)
						== openWindows.end()) {
					cv::namedWindow(displayQueue.back().name,
							cv::WINDOW_NORMAL);
...
void displayImage(const char* windowName, const cv::Mat& image, bool autoSize) {
	log("------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage START", 2);

	if (useImageDisplayThread) {
		log("------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage STEP 1", 2);
		...
		log("debug: ------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage STEP 2", 2);
		openCVdisplaySignal.notify_one();
	} else {
		log("------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage STEP 3", 2);
		if (!autoSize) {
		...
		log("------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage STEP 4", 2);
		log(windowName, 2);
		log("------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage STEP 4.1", 2);
		cv::imshow(windowName, image);
	}
	log("------------------>>>>>>>>>>>>>>>>> ImageDisplay_OpenCV::displayImage STEP 5 --- END ", 2);
	//cv::waitKey(1);
}
