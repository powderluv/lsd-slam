...
DepthMap::DepthMap(int w, int h, const Eigen::Matrix3f& K) {
	...
	nAvgObserve = nAvgRegularize = nAvgPropagate = nAvgFillHoles =
			nAvgSetDepth = 0;
}
...
////	log("=============>>>>>>> DepthMap::observerDepthRow ########## START STEP1", 2);
...
////	log("=============>>>>>>> DepthMap::observerDepthRow ########## END STEP2", 2);

}
void DepthMap::observeDepth() {

///	log("=============>>>>>>> DepthMap::observerDepth ########## START STEP1", 2);
	threadReducer.reduce(
			boost::bind(&DepthMap::observeDepthRow, this, _1, _2, _3), 3,
			height - 3, 10);

	if (enablePrintDebugInfo && printObserveStatistics) {
		log("=============>>>>>>> DepthMap::observerDepth ########## START STEP2", 2);
		printf(
				"OBSERVE (%d): %d / %d created; %d / %d updated; %d skipped; %d init-blacklisted\n",
	...
	if (enablePrintDebugInfo && printObservePurgeStatistics) {
		log("============>>>>>>> DepthMap::observerDepth ########## START STEP3", 2);
		printf(
...
////	log("=============>>>>>>> DepthMap::observerDepth ########## END STEP3", 2);
}
...
//	log("=============>>>>>>> DepthMap::obsererDepthCreate ########## START STEP1", 2);
	DepthMapPixelHypothesis* target = currentDepthMap + idx;

	Frame* refFrame =
			activeKeyFrameIsReactivated ?
	...
	float error = doLineStereo(new_u, new_v, epx, epy, 0.0f, 1.0f,
			1.0f / MIN_DEPTH, refFrame, refFrame->image(0), result_idepth,
	...
	*target = DepthMapPixelHypothesis(result_idepth, result_var,
	...
	if (plotStereoImages)
		debugImageHypothesisHandling.at < cv::Vec3b > (y, x) = cv::Vec3b(255,
				255, 255); // white for GOT CREATED

	if (enablePrintDebugInfo)
		stats->num_observe_created++;

//	log("=============>>>>>>> DepthMap::obsererDepthCreate ########## END STEP2", 2);
...
bool DepthMap::observeDepthUpdate(const int &x, const int &y, const int &idx,
	...
	float error = doLineStereo(x, y, epx, epy, min_idepth,
			target->idepth_smoothed, max_idepth, refFrame, refFrame->image(0),
	...
	if (error == -1) {
		...
		if (plotStereoImages)
			debugImageHypothesisHandling.at < cv::Vec3b > (y, x) = cv::Vec3b(0,
					0, 255);	// RED FOR OOB
		return false;
	}
	...
	// if just not good for stereo (e.g. some inf / nan occured; has inconsistent minimum; ..)
		...
		if (plotStereoImages)
			debugImageHypothesisHandling.at < cv::Vec3b > (y, x) = cv::Vec3b(
					255, 0, 255);	// PURPLE FOR NON-GOOD
	...
	// if not found (error too high)
		...
		if (plotStereoImages)
			debugImageHypothesisHandling.at < cv::Vec3b > (y, x) = cv::Vec3b(0,
					0, 0);	// BLACK FOR big arithmetic error
	...
	// if inconsistent
		...
		if (plotStereoImages)
			debugImageHypothesisHandling.at < cv::Vec3b > (y, x) = cv::Vec3b(
					255, 255, 0);	// Turkoise FOR big inconsistent
		...
		if (target->validity_counter
			...
			target->validity_counter = VALIDITY_COUNTER_MAX
					+ absGrad * (VALIDITY_COUNTER_MAX_VARIABLE) / 255.0f;
		...
		if (plotStereoImages)
			debugImageHypothesisHandling.at < cv::Vec3b > (y, x) = cv::Vec3b(0,
					255, 255); // yellow for GOT UPDATED
...
void DepthMap::propagateDepth(Frame* new_keyframe) {
	log("=============>>>>>>> DepthMap::propagateDepth ########## START STEP1", 2);
	...
	Eigen::Matrix3f trafoInv_R = oldToNew_SE3.rotationMatrix().matrix().cast<
			float>();
	...
	for (int y = 0; y < height; y++)
		for (int x = 0; x < width; x++) {
			...
			Eigen::Vector3f pn = (trafoInv_R
					* Eigen::Vector3f(x * fxi + cxi, y * fyi + cyi, 1.0f))
					/ source->idepth_smoothed + trafoInv_t;
			...
			if (trackingWasGood != 0) {
				...
				float destColor = getInterpolatedElement(newKFImageData, u_new,
						v_new, width);
				...
				if (residual * residual
						...
						> 1 || destAbsGrad < MIN_ABS_GRAD_DECREASE) {
					if (enablePrintDebugInfo)
						runningStats.num_prop_removed_colorDiff++;
					continue;
				}
			...
			if (!targetBest->isValid) {
				...
				*targetBest = DepthMapPixelHypothesis(new_idepth, new_var,
				...
				float merged_new_idepth = w * targetBest->idepth
						+ (1 - w) * new_idepth;

				// merge validity
				int merged_validity = source->validity_counter
						+ targetBest->validity_counter;
				if (merged_validity
						> VALIDITY_COUNTER_MAX
	...
	if (enablePrintDebugInfo && printPropagationStatistics) {
		printf(
				"PROPAGATE: %d: %d drop (%d oob, %d color); %d created; %d merged; %d occluded. %d col-dec, %d grad-dec.\n",
				runningStats.num_prop_attempts,
	...
	log("=============>>>>>>> DepthMap::propagateDepth ########## END STEP2", 2);
}
	...
	for (int y = yMin; y < yMax; y++) {
		for (int x = 3; x < width - 2; x++) {
			...
			if ((dest->blacklisted >= MIN_BLACKLIST
				...
				DepthMapPixelHypothesis* s1max = otherDepthMap + (x - 2)
						+ (y + 3) * width;
				...
				currentDepthMap[idx] = DepthMapPixelHypothesis(idepthObs,
				VAR_RANDOM_INIT_INITIAL, 0);
...
void DepthMap::regularizeDepthMapFillHoles() {
	...
	threadReducer.reduce(
			boost::bind(&DepthMap::regularizeDepthMapFillHolesRow, this, _1, _2,
					_3), 3, height - 2, 10);
...
void DepthMap::buildRegIntegralBuffer() {
	threadReducer.reduce(
			boost::bind(&DepthMap::buildRegIntegralBufferRow1, this, _1, _2,
					_3), 0, height);
	...
	for (int y = yMin; y < yMax; y++) {
		for (int x = regularize_radius; x < width - regularize_radius; x++) {
			...
			for (int dx = -regularize_radius; dx <= regularize_radius; dx++)
				for (int dy = -regularize_radius; dy <= regularize_radius;
						dy++) {
					DepthMapPixelHypothesis* source = destRead + dx
							+ dy * width;
...
void DepthMap::regularizeDepthMap(bool removeOcclusions, int validityTH) {
	...
	if (removeOcclusions)
		threadReducer.reduce(
				boost::bind(&DepthMap::regularizeDepthMapRow<true>, this,
						validityTH, _1, _2, _3), 2, height - 2, 10);
	else
		threadReducer.reduce(
				boost::bind(&DepthMap::regularizeDepthMapRow<false>, this,
						validityTH, _1, _2, _3), 2, height - 2, 10);

	if (enablePrintDebugInfo && printRegularizeStatistics)
		printf(
				"REGULARIZE (%d): %d smeared; %d blacklisted /%d new); %d deleted; %d occluded; %d filled\n",
...
void DepthMap::initializeRandomly(Frame* new_frame) {
	log("-------------------------------->>>>>>>>>>>>>>>> DepthMap::initializeRandomly ------- START STEP 1", 2);
	...
	log("-------------------------------->>>>>>>>>>>>>>>> DepthMap::initializeRandomly ------- STEP 3", 2);

	activeKeyFrame->setDepth(currentDepthMap);

	log("-------------------------------->>>>>>>>>>>>>>>> DepthMap::initializeRandomly ------- END STEP 4", 2);
}
...
void DepthMap::updateKeyframe(
	...
	msObserve = 0.9 * msObserve + 0.1 * std::chrono::duration_cast
			< std::chrono::milliseconds > (tv_end - tv_start).count();
	nObserve++;
	...
	//if(rand()%10==0)
	{
		...
		// gettimeofday(&tv_end, NULL);
		tv_end = std::chrono::high_resolution_clock::now();
		msFillHoles = 0.9 * msFillHoles + 0.1 * std::chrono::duration_cast
				< std::chrono::milliseconds > (tv_end - tv_start).count();
		nFillHoles++;
	}
	...
	// gettimeofday(&tv_end, NULL);
	tv_end = std::chrono::high_resolution_clock::now();
	msRegularize = 0.9 * msRegularize + 0.1 * std::chrono::duration_cast
			< std::chrono::milliseconds > (tv_end - tv_start).count();
	nRegularize++;
	...
	if (!activeKeyFrame->depthHasBeenUpdatedFlag) {
		...
		// gettimeofday(&tv_end, NULL);
		tv_end = std::chrono::high_resolution_clock::now();
		msSetDepth = 0.9 * msSetDepth + 0.1 * std::chrono::duration_cast
				< std::chrono::milliseconds > (tv_end - tv_start).count();
		nSetDepth++;
	}
	...
	msUpdate = 0.9 * msUpdate + 0.1 * std::chrono::duration_cast
			< std::chrono::milliseconds > (tv_end_all - tv_start_all).count();
	nUpdate++;
	...
	if (plotStereoImages) {
		Util::displayImage("Stereo Key Frame", debugImageHypothesisHandling,
				false);
		Util::displayImage("Stereo Reference Frame", debugImageStereoLines,
				false);
	}
...
void DepthMap::createKeyFrame(Frame* new_keyframe) {
	log("debug: =============>>>>>>> DepthMap::createKeyFrame ########## START STEP1\n", 1);
	...
	boost::shared_lock < boost::shared_mutex > lock2 =
			new_keyframe->getActiveLock();
	...
	// gettimeofday(&tv_end, NULL);
	tv_end = std::chrono::high_resolution_clock::now();
	msPropagate = 0.9 * msPropagate + 0.1 * std::chrono::duration_cast
			< std::chrono::milliseconds > (tv_end - tv_start).count();
	nPropagate++;
	...
	//gettimeofday(&tv_end_all, NULL);
	tv_end_all = std::chrono::high_resolution_clock::now();
	msCreate = 0.9 * msCreate + 0.1 * std::chrono::duration_cast
			< std::chrono::milliseconds > (tv_end_all - tv_start_all).count();
	nCreate++;
	...
	printf(
			"debug: =============>>>>>>> DepthMap::createKeyFrame ########## END STEP2\n");
}
...
void DepthMap::addTimingSample() {
	...
	if (sPassed > 1.0f) {
		...
		if (enablePrintDebugInfo && printMappingTiming) {
			printf(
					"Upd %3.1fms (%.1fHz); Create %3.1fms (%.1fHz); Final %3.1fms (%.1fHz) // Obs %3.1fms (%.1fHz); Reg %3.1fms (%.1fHz); Prop %3.1fms (%.1fHz); Fill %3.1fms (%.1fHz); Set %3.1fms (%.1fHz)\n",
					msUpdate, nAvgUpdate, msCreate, nAvgCreate, msFinalize,
					nAvgFinalize, msObserve, nAvgObserve, msRegularize,
					nAvgRegularize, msPropagate, nAvgPropagate, msFillHoles,
					nAvgFillHoles, msSetDepth, nAvgSetDepth);
		}
	}
}
...
void DepthMap::finalizeKeyFrame() {
	printf(
			"debug: =============>>>>>>> DepthMap::finalizeKeyFrame ########## START STEP1\n");

	assert(isValid());

	printf(
			"debug: =============>>>>>>> DepthMap::finalizeKeyFrame ########## START STEP2\n");
	...
	//gettimeofday(&tv_end_all, NULL);
	tv_end_all = std::chrono::high_resolution_clock::now();
	msFinalize = 0.9 * msFinalize + 0.1 * std::chrono::duration_cast
			< std::chrono::milliseconds > (tv_end_all - tv_start_all).count();
	nFinalize++;

	printf(
			"debug: =============>>>>>>> DepthMap::finalizeKeyFrame ########## END STEP3\n");
}

int DepthMap::debugPlotDepthMap() {
	log("=============>>>>>>> DepthMap::debugPlotDepthMap ########## START STEP1", 2);
	log("debug: =============>>>>>>> DepthMap::debugPlotDepthMap ########## START STEP1 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", 1);
	...
	log("=============>>>>>>> DepthMap::debugPlotDepthMap ########## STEP2", 2);
	...
	log("=============>>>>>>> DepthMap::debugPlotDepthMap ########## END STEP3", 2);
...
inline float DepthMap::doLineStereo(const float u, const float v,
		const float epxn, const float epyn, const float min_idepth,
	...
	Eigen::Vector3f pReal = pInf / prior_idepth
	...
	float realVal_m1 = getInterpolatedElement(activeKeyFrameImageData,
			u - epxn * rescaleFactor, v - epyn * rescaleFactor, width);
	float realVal = getInterpolatedElement(activeKeyFrameImageData, u, v,
			width);
...
//	if(referenceFrame->K_otherToThis_t[2] * max_idepth + pInf[2] < 0.01)
	...
	float val_cp_m2 = getInterpolatedElement(referenceFrameImage,
	...
	if (abs(loopCBest - loopCSecond) > 1
			&& MIN_DISTANCE_ERROR_STEREO * best_match_err
	...
	float photoDispError = 4 * cameraPixelNoise2
			/ (gradAlongLine + DIVISION_EPS);

	float trackingErrorFac = 0.25
			* (1 + referenceFrame->initialTrackedResidual);
